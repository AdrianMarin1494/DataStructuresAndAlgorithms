...
function breakingRecords(scores) {
    ...
    // iterate over the scores, track the lowest and maximum, make a counter for minimum and one for maximum
    let minimum = scores[0];
    let maximum = scores[0];
    let counterOfMinimumRecords = 0;
    let counterOfMaximumRecords = 0;
    for (let score of scores) {
        if (minimum < score) {
            minimum = score;
            counterOfMinimumRecords++;
        }
        if (maximum > score) {
            maximum = score;
            counterOfMaximumRecords++;
        }
    }
    const result = [];
    result.unshift(counterOfMinimumRecords);
    result.push(counterOfMaximumRecords);
    console.log(result);
    return result;
}
